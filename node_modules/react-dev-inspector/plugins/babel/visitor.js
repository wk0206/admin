...
const doJSXOpeningElement = (node, option) => {
    ...
    const lineAttr = isNil(line)
        ? null
        : generated_1.jsxAttribute(generated_1.jsxIdentifier('data-inspector-line'), generated_1.stringLiteral(line.toString()));
    const columnAttr = isNil(column)
        ? null
        : generated_1.jsxAttribute(generated_1.jsxIdentifier('data-inspector-column'), generated_1.stringLiteral(column.toString()));
    const relativePathAttr = generated_1.jsxAttribute(generated_1.jsxIdentifier('data-inspector-relative-path'), generated_1.stringLiteral(relativePath));
...
const createVisitor = ({ cwd, excludes }) => {
    const isExclude = (excludes === null || excludes === void 0 ? void 0 : excludes.length)
        ? memo((filePath) => exports.pathMatch(filePath, excludes))
        : () => false;
    const pathRelative = memo((filePath) => path_1.relative(cwd !== null && cwd !== void 0 ? cwd : process.cwd(), filePath));
    const visitor = {
        JSXOpeningElement: {
            enter(path, state) {
                var _a, _b;
                ...
                exports.doJSXOpeningElement(path.node, {
                    relativePath,
                });
            },
        },
