 ...
 */
var _default = api => {
  const hostPkgAlias = (0, _getHostPkgAlias.default)(api.paths).filter(([pkgName]) => pkgName); // create symlink for packages
  ...
  hostPkgAlias.forEach(([pkgName, pkgPath]) => {
    ...
    if (!_fs().default.existsSync(linkPath) || _fs().default.lstatSync(linkPath).isSymbolicLink() && _fs().default.readlinkSync(linkPath) !== pkgPath) {
